...
SET BL_SD_HEX=merged_SD_bootloader.hex

echo "## Looking to make sure %BL_SD_HEX% is present in folder"
if not exist %BL_SD_HEX% (
echo "#### merged_SD_bootloader.hex file does not exist! Please copy a application .hex file into the folder, rename it, and try again!"
goto :end
)
echo.
...
echo "## Merging merged_SD_bootloader.hex and merged.hex into super.hex"
mergehex -m merged.hex merged_SD_bootloader.hex -o super.hex
echo.

nrfjprog --program super.hex --chiperase
